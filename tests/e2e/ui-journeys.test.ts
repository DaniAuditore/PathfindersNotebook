...
test("recupera sólo el correo tras un error de acceso y enfoca el aviso seguro", async ({ page, context }) => {
  await context.addCookies([{
    name: "login_recovery_email",
  ...
  await expect(error).toContainText("No pudimos iniciar sesión.");
  await expect(page.getByLabel("Correo electrónico")).toHaveValue("learner@example.test");
